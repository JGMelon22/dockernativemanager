...
import { getNetworks, deleteNetwork, createNetwork, Network } from "@/lib/docker";
import { 
  Table, 
  TableBody, 
  TableCell, 
    ...
    <>
      <div className="space-y-6">
        <div className="flex justify-between items-center">
          <div>
            <h2 className="text-3xl font-bold text-white tracking-tight">Networks</h2>
            <p className="text-zinc-500 mt-1">Manage virtual networks for container communication.</p>
          </div>
          ...
          <Table>
            <TableHeader className="bg-zinc-900/80">
              <TableRow className="border-zinc-800 hover:bg-transparent">
                ...
                <TableHead className="text-zinc-400 font-medium">Network ID</TableHead>
                <TableHead className="text-zinc-400 font-medium">Driver</TableHead>
                <TableHead className="text-zinc-400 font-medium">Scope</TableHead>
                <TableHead className="text-zinc-400 font-medium text-right">Actions</TableHead>
              </TableRow>
            </TableHeader>
            <TableBody>
              {filtered.map((n) => (
                <TableRow key={n.id} className="border-zinc-800 hover:bg-zinc-800/30 transition-colors">
                  ...
                  <TableCell className="text-zinc-500 text-xs font-mono">{n.id}</TableCell>
                  <TableCell>
                    <span className="bg-zinc-800 text-zinc-400 text-[10px] px-2 py-0.5 rounded border border-zinc-700 font-mono">
                      {n.driver}
                    </span>
                  </TableCell>
                  <TableCell className="text-zinc-500 text-xs capitalize">{n.scope}</TableCell>
